...
def parse_pdf(path):
    ...
    d["bdv"] = ev(
    full,
    r"Electric Strength \(BDV\).*?([0-9.]+)\s+30"
    )

    d["water"] = ev(
    full,
    r"Water Content By Karl Fischer Method.*?([0-9.]+)\s+40"
    )

    d["neutralization"] = ev(
    full,
    r"Neutralization Value.*?([0-9.]+)\s+0\.3"
    )

    d["ift"] = ev(
    full,
    r"Interfacial Tension.*?([0-9.]+)\s+0\.020"
    )
