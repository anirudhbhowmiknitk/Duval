...
# Gas aliases: key used internally → list of label strings found in reports
GAS_ALIASES = {
    "H2":   ["H2", "H₂", "Hydrogen"],
    "O2":   ["O2", "O₂", "Oxygen"],
    "N2":   ["N2", "N₂", "Nitrogen"],
    "CO":   ["CO ", "Carbon Monoxide", "CO\t", "CO\n"],   # space avoids CO2 match
    "CH4":  ["CH4", "CH₄", "Methane"],
    "CO2":  ["CO2", "CO₂", "Carbon Dioxide"],
    "C2H4": ["C2H4", "C₂H₄", "Ethylene"],
    "C2H6": ["C2H6", "C₂H₆", "Ethane"],
    "C2H2": ["C2H2", "C₂H₂", "Acetylene"],
    "C3H6": ["C3H6", "C₃H₆", "Propylene"],
    "C3H8": ["C3H8", "C₃H₈", "Propane"],
    "BDV":  ["BDV", "Breakdown Voltage", "Dielectric Strength"],
    "Water":["Water Content", "Water", "Moisture", "H2O"],
}

# Meta field aliases
META_ALIASES = {
    "equipment":    ["Equipment", "Transformer", "Asset", "Apparatus", "Unit"],
    "serial":       ["Serial No", "Serial Number", "S/N", "Serial"],
    "manufacturer": ["Manufacturer", "Make", "Vendor", "OEM"],
    "location":     ["Location", "Substation", "Station", "Site", "Address"],
    "date":         ["Date", "Test Date", "Sample Date", "Sampling Date"],
    "voltage":      ["Voltage", "Rating", "kV Rating"],
    "mva":          ["MVA", "Capacity", "KVA"],
}


def _first_float(text):
    """Extract first float-like token from a string."""
...
def load_file(path):
    ...
    gases = {
        ...
        "BDV": extract_number(
            r"Electric Strength \(BDV\).*?\s(\d+\.?\d*)",
            text
        ),

        "Water": extract_number(
            r"Water Content By Karl Fischer.*?\s(\d+\.?\d*)",
            text
        ),
...
def draw(gases, meta, out_path):
    ...
    if BDV is not None:
        row(axI, y, "BDV:", f"{BDV:.1f}", "kV", warn=BDV < 40); y -= 0.038
    else:
        row(axI, y, "BDV:", "N/A"); y -= 0.038
    if WC is not None:
        row(axI, y, "Water Content:", f"{WC:.1f}", "ppm", warn=WC > 30); y -= 0.038
    else:
    ...
    for name, val, lim in gas_rows:
        ...
        row(axI, y, f"  {name}", f"{val:.2f}", "ppm", warn=over)
        y -= 0.035
    ...
    if co2co is not None:
        row(axI, y, "  CO₂/CO Ratio:", f"{co2co:.2f}");    y -= 0.035
    row(axI, y, "  TDCG Level:",
        f"Level {tdcg_lv}  —  {tdcg_st}",
        warn=tdcg_lv >= 3, hi=tdcg_lv < 3);               y -= 0.050
...
def main():
    ...
    # ── extract data ─────────────────────────────────────────────────────────
    gases, meta = load_file(file_path)
