...
def parse_pdf(path):
    ...
    # DGA Gases
    d["h2"] = ev(full, r"Hydrogen \(H2.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["o2"] = ev(full, r"Oxygen \(O2.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["n2"] = ev(full, r"Nitrogen \(N2.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["ch4"] = ev(full, r"Methane \(CH4.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["c2h6"] = ev(full, r"Ethane \(C2H6.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["c2h4"] = ev(full, r"Ethylene \(C2H4.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["c2h2"] = ev(full, r"Acetylene \(C2H2.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    d["co"] = ev(full, r"Carbon Monoxide \(CO\).*?µl/l\s*([\d.]+|BDL)")
    d["co2"] = ev(full, r"Carbon Dioxide \(CO2.*?\)\s*,\s*µl/l\s*([\d.]+|BDL)")
    ...
    comments = re.findall(r"\d+\]\s*(.*)", full)

    if comments:
        d["recommendation"] = " | ".join(comments)
    else:
        d["recommendation"] = ""
